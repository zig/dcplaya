/*
 * 64bit aligned/packed access motion 
 *
...
#define	LP(p)	*(uint64_t*)(p)

#define	BYTE_VEC(c)	((c)*0x0101010101010101ULL)

#define	UNPACK(ph,pl,tt0,tt1) do { \
	uint64_t t0,t1; t0=tt0;t1=tt1; \
...
/* little endian */
#define	MERGE1(a,b,ofs)	(ofs==0)?a:( ((a)>>(8*ofs))|((b)<<(64-8*ofs)) )
#define	MERGE2(a,b,ofs)	(ofs==7)?b:( ((a)>>(8*(ofs+1)))|((b)<<(64-8*(ofs+1))) )
#else
/* big */
#define	MERGE1(a,b,ofs)	(ofs==0)?a:( ((a)<<(8*ofs))|((b)>>(64-8*ofs)) )
#define	MERGE2(a,b,ofs)	(ofs==7)?b:( ((a)<<(8*(ofs+1)))|((b)>>(64-8*(ofs+1))) )
...
static inline uint64_t rnd_avg2(uint64_t a, uint64_t b)
{
    return (a | b) - (((a ^ b) & ~BYTE_VEC(0x01)) >> 1);
}

static inline uint64_t no_rnd_avg2(uint64_t a, uint64_t b)
{
    return (a & b) + (((a ^ b) & ~BYTE_VEC(0x01)) >> 1);
}

#define	LP32(p)	*(uint32_t*)(p)
#define	put32(d,s)	d = s
#define	avg32(d,s)	d = rnd_avg32(s,d)
...
#define	OP_C4(ofs) \
	ref-=ofs; \
	do { \
		OP(LP32(dest),MERGE1(LP(ref),0ULL,ofs)); \
		ref+=stride; \
		dest+=stride; \
	} while(--height)
...
static void put_pixels4_c(uint8_t *dest,const uint8_t *ref, const int stride,int height)
{
	...
	case 5: OP_C4(5); return;
	case 6: OP_C4(6); return;
	case 7: OP_C4(7); return;
	}
}
...
if (sz==16) { \
		t0 = LP(ref+16); \
		OP(LP(dest+8), avg2(MERGE1(t1,t0,ofs),MERGE2(t1,t0,ofs))); \
} \
		ref+=stride; \
		dest+= stride; \
	} while(--height); \
...
{ \
	...
	case 4:OP_N(4,sz,rnd##_##avgfunc); return; \
	case 5:OP_N(5,sz,rnd##_##avgfunc); return; \
	case 6:OP_N(6,sz,rnd##_##avgfunc); return; \
	case 7:OP_N(7,sz,rnd##_##avgfunc); return; \
	} \
}
...
//#define	QPEL this isn't optiimize 64bit now
...
void dsputil_init_align64(DSPContext* c, AVCodecContext *avctx)
{
