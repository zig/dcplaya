...
$Id: mhead.c,v 1.2 2002-12-22 19:17:35 ben Exp $
_________________________________________________________________________*/
...
int head_info(unsigned char *buf, unsigned int n, MPEG_HEAD * h)
{
  ...
  if (!framebytes) {
    /* ben : whatever happens later, if we had no framebytes try to find a
       free format !*/
...
int head_info3(unsigned char *buf, unsigned int n, MPEG_HEAD *h,
	       int *br, unsigned int *searchForward) {
  unsigned int pBuf;
  // jdw insertion...
  // ben fix
  for (pBuf = 0; n>1; --n, ++pBuf) {
    int frame_bytes;
    /*     if ((buf[pBuf] == 0xFF) &&  */
    /* 	((buf[pBuf+1] & 0xF0) == 0xF0 || (buf[pBuf+1] & 0xF0) == 0xE0)) { */
    /* ben: Add this test becoz some file has JPEG (or other data) inside that
       matchs mpeg poor magic number ! */
    frame_bytes = head_info2(buf+pBuf,n,h,br);
    if (frame_bytes > 0) {
      *searchForward = pBuf;
      return frame_bytes;
    }
...
{
  return buf[0] == buf2[0] && buf[1] == buf2[1];
}
/*----------------------------------------------------------*/
