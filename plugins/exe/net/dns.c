...
int lwip_gethostbyname(const struct sockaddr_in * dnssrv, const char * name, uint8 * ipout) {
	...
	sock = lwip_socket(PF_INET, SOCK_DGRAM, 0);
	if (sock < 0)
		return -1;
	...
	if (lwip_sendto(sock, qb, size, 0, (struct sockaddr *)&toaddr, addrsize) <= 0)
		return -1;

	// Get the response.
	if ((size = lwip_recvfrom(sock, qb, 1500, 0, (struct sockaddr *)&toaddr, &addrsize)) <= 0)
		return -1;
...
int lwip_gethostbyname2(const char * srv, const char * name, uint8 * ipout) {
	uint8	qb[1500];
	int 	size;
	struct netconn * conn;
	struct netbuf * buf;
	...
	if (netconn_connect(conn, &addr, 53))
	  return -1;

	// Send the query to the server.
	printf("sending request ...\n");
	buf = netbuf_new();
	netbuf_ref(buf, qb, size);
	//*netbuf_fromaddr(buf) = addr;
	if (netconn_send(conn, buf) < 0)
	  return -1;
