...
/*  Routines include:
    ...
    FFT2real : Radix-2 FFT, with real data assumed,
...
{
  long j;
  float a = 0.0f, inc = 2.0f * (float) PI / (float) n;
  LNODE *plnode;
  complex *expn;
  ...
  if (ex != NULL)
    ...
    expn = (complex *) malloc((long) n * sizeof(complex));
    if (expn == NULL)
      return (NULL);
  }
  for (j = 0; j < n; j++) {
    a = j * inc;
    expn[j].re = (float) Cos(a);
    expn[j].im = -(float) Sin(a);
  }
  plnode = lroot;
  lroot = (LNODE *) malloc((long) sizeof(LNODE));
  lroot->next = plnode;
  lroot->size = n;
  lroot->table = expn;
...
void FFT2torl(									/* Performs FFT on data assumed complex conjugate */
	      ...
	      long n, int skip, float scale,	/* (frigged for ugens7.c) */
	      complex ex[])
{																/* pass in lookup table */
  ...
  for (mm = half - 1; mm >= 0; mm--) {
    ...
    x[m << 1].im = x[(m << 1) + skip].im = 0.0f;
  }
}

void FFT2torlpacked(complex x[], long n, float scale, complex ex[])
...
void ConjScale(complex x[], long len, float scale)
...
void Reals(											/* sign is 1 for FFT2real, -1 for torl */
	   complex x[], long n, int skip, int sign, complex ex[])
{																/* lookup table passed in */
  ...
  for (mm = 0; mm <= quarter; mm++) {
    m = mm * skip;
    s.re = (1.0f + ex[mm].im) / 2.0f;
    s.im = (-(float) sign) * ex[mm].re / 2.0f;
    t.re = 1.0f - s.re;
    t.im = -s.im;
    a = x[m];
    b = x[half - m];
...
void Realspacked(								/* sign is 1 for FFT2real, -1 for torl */
		 ...
		 complex x[], long n, int sign, complex ex[])
{																/* lookup table passed in */
  int half = (int) (n >> 1);
  int quarter = half >> 1;
  int m;
  ...
  if (sign == 1)
    x[half] = x[0];							/* only for Yc to Xr */
  for (m = 0; m <= quarter; m++) {
    s.re = (1.0f + ex[m].im) / 2.0f;
    s.im = (-(float) sign) * ex[m].re / 2.0f;
    t.re = 1.0f - s.re;
    t.im = -s.im;
    a = x[m];
    b = x[half - m];
...
void FFT2(
	  ...
	  */
	  complex x[], long n, int skip, complex ex[])
{
  FFT2raw(x, n, 1, skip, ex);
  reverseDig(x, n, skip);
}

void FFT2raw(complex x[],				/* Data is x */
